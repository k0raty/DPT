...
while(compteur<100): #Infinit loop
    ...
    if(serialPort.in_waiting > 0):
        ...
        # Read data out of the buffer until a carraige return / new line is found
           ...
           # print(serialList)
           # print("là" ,serialList.find("]"))
            if serialList.find("[") != -1:
                BigSerialList = "["
            if BigSerialList.find("]") == -1:           
                BigSerialList += serialList
             #   print("super",BigSerialList)
              #  print()
            else : #Fin de liste
                BigSerialList = BigSerialList[2:]
                BigSerialList = BigSerialList[:-3]
                print()
                print(BigSerialList)
                L = []
                k = 0
                break
                while BigSerialList.find(",") != -1:
                    if(BigSerialList[BigSerialList.find(",")-1]==0) :
                        L.append[0]
                    else:
                        L.append(float(BigSerialList[BigSerialList.find(",")-3:BigSerialList.find(",")]))
                    BigSerialList=BigSerialList[BigSerialList.find(",")+1:]
          ...
          #      L = list(map(remap,L))
                ...
                for i in range(size) :
                    for j in range(size) :
                        ...
                        k+=1
                print()
                print("Matrice ici !" ,serialMatrix)
